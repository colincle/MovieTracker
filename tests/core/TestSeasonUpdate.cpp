...
class TestSeasonUpdate : public QObject
{
  ...
  private slots:
    ...
    void isNotEmptyWhenSeriesCheckedYesterday()
    {
        AppStorage storage;
        storage.addTitle(
            makeSeries("tt0000001", QDate::currentDate().addDays(-1)), QPixmap{}
        );
        SeasonUpdate su(storage);
        QVERIFY(!su.isEmpty());
    }
    ...
    {
        AppStorage storage;
        storage.addTitle(makeSeries("tt0000001", QDate::currentDate()), QPixmap{}); // not eligible: checked today
        storage.addTitle(makeSeries("tt0000002", QDate{}), QPixmap{});              // eligible: never checked
        storage.addTitle(makeMovie("tt0000003"), QPixmap{});                         // not eligible: movie
        storage.addTitle(
            makeSeries("tt0000004", QDate::currentDate().addDays(-1)), QPixmap{}
    ...
    {
        if(!hasApiKey()) QSKIP("Set OMDB_API_KEY to run integration tests");
        AppStorage storage;
        storage.setOmdbApiKey(apiKey());
        storage.addTitle(makeSeries("tt0944947", QDate{}, "8"), QPixmap{}); // Game of Thrones
        SeasonUpdate su(storage);
        QVERIFY(!su.isEmpty());
    ...
    void updateSeriesUpdatesLastCheckedToToday()
    {
        ...
        storage.setOmdbApiKey(apiKey());
        storage.addTitle(
            makeSeries("tt0944947", QDate::currentDate().addDays(-1), "8"), QPixmap{}
        );
        SeasonUpdate su(storage);
        su.updateSeries();
...
QObject *createTestSeasonUpdate() { return new TestSeasonUpdate(); }
