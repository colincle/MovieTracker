...
class TestSeasonUpdateController : public QObject
{
  ...
  private slots:
    ...
    void updateStartedFiresBeforeUpdateFailed()
    {
        ...
        QStringList order;
        connect(
            &controller, &SeasonUpdateController::updateStarted,
            [&order]() { order << "started"; }
        );
        connect(
            &controller, &SeasonUpdateController::updateFailed,
...
QObject *createTestSeasonUpdateController() { return new TestSeasonUpdateController(); }
