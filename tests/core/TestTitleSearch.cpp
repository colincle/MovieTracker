  ...
  private slots:
	...
	void sortAlphaAZSortsAZ()
	{
		std::vector<Title> titles =
		    {makeTitle("Zorro"), makeTitle("Alien"), makeTitle("Batman")};
	...
	void sortAlphaAZStripsLeadingAn()
	{
		std::vector<Title> titles = {
		    makeTitle("Zorro"),
		    makeTitle("An Inconvenient Truth")
		};
		sortTitles(titles, SortMode::AlphaAZ);
		QCOMPARE(
		    titles[0].title,
		    "An Inconvenient Truth"
		); // sorts as "inconvenient truth"
		QCOMPARE(titles[1].title, "Zorro");
	}
...
QObject *createTestTitleSearch()
{
	return new TestTitleSearch();
}
