...
class TestImportedFileValidator : public QObject
{
  ...
  private slots:
    ...
    {
        ValidationResult result = ImportedFileValidator::validate("/nonexistent/path.zip");
    ...
    void validateTitleInvalidTypeIsInvalid()
    {
        ...
        QVERIFY(tmp.isValid());
        QString zipPath = tmp.path() + "/test.zip";
        QVERIFY(createLibraryZip(tmp.path(), jsonWithTitleField("type", "\"documentary\""), zipPath));
    ...
    void validateTitleTypeSeries()
    {
        ...
        QVERIFY(tmp.isValid());
        QString zipPath = tmp.path() + "/test.zip";
        QVERIFY(createLibraryZip(tmp.path(), jsonWithTitleField("type", "\"series\""), zipPath));
    ...
    void validateRequiredStringFields()
    {
        const QStringList fields = {
            "actors", "director", "imdbId", "lastChecked", "lastViewed",
            "plot", "released", "title", "totalSeasons", "type", "year"
        };
        ...
        for(const QString &field : fields)
        {
            ...
            ValidationResult result = ImportedFileValidator::validate(zipPath);
            QVERIFY2(!result.valid, qPrintable("Expected failure for missing field: " + field));
            QVERIFY2(result.error.contains(field), qPrintable("Error should mention: " + field));
        }
    }
  ...
  private:
    ...
    {
        return QDir(AppPaths::dataDir()).dirName();
    }
    ...
    static bool zipIt(
        const QString &workingDir, const QStringList &entries, const QString &zipPath
    )
    {
        QProcess p;
...
QObject *createTestImportedFileValidator() { return new TestImportedFileValidator(); }
