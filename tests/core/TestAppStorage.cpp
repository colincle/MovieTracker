...
class TestAppStorage : public QObject
{
  ...
  private slots:
    ...
    {
        { AppStorage s; s.addTitle(makeTitle("tt0000001"), QPixmap{}); }
        AppStorage reloaded;
    ...
    void addTitleSetsViewedFalse()
    {
        ...
        t.viewed = true;
        { AppStorage s; s.addTitle(t, QPixmap{}); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.addTitle(makeTitle("tt0000001"), QPixmap{}); }
        { AppStorage s; s.deleteTitle("tt0000001"); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.addTitle(makeTitle("tt0000001"), QPixmap{}); }
        { AppStorage s; s.toggleViewed("tt0000001"); }
        AppStorage reloaded;
        QVERIFY(reloaded.getTitles()[0].viewed);
    ...
    {
        { AppStorage s; s.addTitle(makeTitle("tt0000001", "movie"), QPixmap{}); }
        { AppStorage s; s.insertRank("tt0000001", 0, "movie"); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.addNotifications({"tt0000001", "tt0000002"}); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.addNotifications({"tt0000001", "tt0000002"}); }
        { AppStorage s; s.removeNotifications(); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.setDarkAccentColor("#aabbcc"); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.setLightAccentColor("#112233"); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.setLibraryCardWidth(220); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.setWindowSize(1280, 900); }
        AppStorage reloaded;
    ...
    {
        { AppStorage s; s.setOmdbApiKey("abc123"); }
        AppStorage reloaded;
    ...
    void addStreamingPlatformPersists()
    {
        {
            AppStorage s;
            s.addStreamingPlatform({"https://netflix.com/search?q=movietracker", "Netflix", ""}, "");
        }
        AppStorage reloaded;
        QCOMPARE((int)reloaded.getStreamingPlatforms().size(), 1);
        QCOMPARE(reloaded.getStreamingPlatforms()[0].name, "Netflix");
        QCOMPARE(reloaded.getStreamingPlatforms()[0].url, "https://netflix.com/search?q=movietracker");
    }
  ...
  private:
    ...
    static std::vector<Title>::const_iterator findById(
        const std::vector<Title> &titles, const QString &imdbId
    )
    {
        return std::find_if(
            titles.begin(), titles.end(), [&](const Title &t) { return t.imdbId == imdbId; }
        );
    }
};
...
QObject *createTestAppStorage() { return new TestAppStorage(); }
