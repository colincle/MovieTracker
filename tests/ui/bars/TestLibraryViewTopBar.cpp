...
class TestLibraryViewTopBar : public QObject
{
  ...
  private slots:
    ...
    void returnPressedQueryIsTrimmed()
    {
        ...
        QString           received;
        connect(
            &bar, &LibraryViewTopBar::searchRequested, [&](const QString &q) { received = q; }
        );
        auto *sb = bar.findChild<SearchBar *>();
        QVERIFY(sb);
        sb->setText("  batman  ");
    ...
    void typingNonEmptyTextDoesNotEmitSearchRequested()
    {
        ...
        int               count = 0;
        connect(
            &bar, &LibraryViewTopBar::searchRequested, [&](const QString &) { ++count; }
        );
        auto *sb = bar.findChild<SearchBar *>();
        QVERIFY(sb);
        sb->setText("B");
...
QObject *createTestLibraryViewTopBar() { return new TestLibraryViewTopBar(); }
