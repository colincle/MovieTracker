...
class TestStreamingPlatformButton : public QObject
{
  ...
  private:
    static StreamingPlatform makePlatform(const QString &name = "Netflix", const QString &image = "")
    {
        StreamingPlatform p;
        p.name  = name;
        p.image = image;
  ...
  private slots:
    ...
    {
        StreamingPlatformButton btn(makePlatform("Netflix", "/nonexistent/image.png"), HEIGHT, COLOR1, COLOR2);
        QVERIFY(btn.icon().isNull());
    }
...
QObject *createTestStreamingPlatformButton() { return new TestStreamingPlatformButton(); }
